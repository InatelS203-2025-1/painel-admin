...
export function UserCollection() {
  ...
  const cardsByType: Record<
    string,
    { cardId: string; pokemon: any; quantity: number; favorite: boolean }[]
  > = {};
  ...
  if (cards.length === 0) {
    return (
      <div className="rounded-lg border border-blue-200 bg-blue-50 p-8 text-center">
        <h2 className="text-lg font-medium text-blue-800">Coleção vazia</h2>
        <p className="mt-2 text-blue-600">
          Quando o jogador adicionar pokémons, aparecerá aqui.
        </p>
      </div>
    );
  }
      ...
      <Tabs defaultValue="all" className="w-full">
        ...
        <TabsContent value="favorites" className="space-y-6">
          {favoriteCards.length === 0 ? (
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-8 text-center">
              ...
              <p className="mt-2 text-blue-600">
                Marque Pokémon como favoritos para vê-los aqui.
              </p>
            </div>
          ) : (
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        ...
        <TabsContent value="by-type" className="space-y-8">
          {Object.keys(cardsByType).length === 0 ? (
            <div className="rounded-lg border border-blue-200 bg-blue-50 p-8 text-center">
              <h2 className="text-lg font-medium text-blue-800">
                Nenhum Pokémon na coleção
              </h2>
              <p className="mt-2 text-blue-600">
                Adicione Pokémon à sua coleção para vê-los aqui.
              </p>
            </div>
          ) : (
              ...
              .map(([typeName, typeCards]) => (
                <div key={typeName} className="space-y-4">
                  <h2 className="flex items-center gap-2 text-xl font-bold text-blue-800">
                    <span
                      className={`inline-block h-4 w-4 rounded-full ${getTypeColor(
                        typeName
                      )}`}
                    ></span>
                    ...
                    <span className="text-sm font-normal text-blue-600">
                      ({typeCards.length}{" "}
                      {typeCards.length === 1 ? "Pokémon" : "Pokémons"})
                    </span>
                  </h2>
                  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                    {typeCards.map((item) =>
