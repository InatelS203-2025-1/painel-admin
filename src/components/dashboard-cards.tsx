...
import { BarChart3, Flame, Sparkles, WalletCardsIcon as Cards } from "lucide-react"
...
export function DashboardCards({ pokemon, isLoading }: DashboardCardsProps) {
  const { cards, collection, getCardCount, getTotalUniqueCards, getCardsByType } = useUserCardStore()
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
      <Card className="border-blue-200 shadow-md">
        <CardHeader className="pb-2">
          <CardDescription>Sua Coleção</CardDescription>
          <CardTitle className="text-2xl">{userStats.totalCards} Cartas</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center">
            <Cards className="mb-3 h-12 w-12 text-blue-400" />
            <div className="text-center">
              <p className="font-medium text-blue-600">{userStats.uniqueCards} Pokémon únicos</p>
              {userStats.topUserTypes.length > 0 ? (
                <div className="mt-2">
                  ...
                  <div className="mt-1 flex flex-wrap justify-center gap-1">
                    {userStats.topUserTypes.map((item) => (
                      <span
                        key={item.type}
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium text-white ${getTypeColor(item.type)}`}
                      >
                ...
                <p className="mt-2 text-sm text-blue-500">Adicione cartas à sua coleção!</p>
              )}
            </div>
          </div>
        </CardContent>
      ...
      <Card className="border-blue-200 shadow-md">
        <CardHeader className="pb-2">
          <CardDescription>Pokémon Disponíveis</CardDescription>
          <CardTitle className="text-2xl">{stats.totalPokemon} Pokémon</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center">
      ...
      <Card className="border-blue-200 shadow-md">
        <CardHeader className="pb-2">
          ...
          <CardTitle className="text-2xl capitalize">{stats.strongestPokemon.name}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center">
            ...
            <div className="text-center">
              ...
              <p className="text-xl font-bold text-blue-700">
                {stats.strongestPokemon.stats.reduce((sum, stat) => sum + stat.base_stat, 0)}
              </p>
              <div className="mt-1 flex justify-center gap-1">
                {stats.strongestPokemon.types.map((type) => (
                  <span
                    key={type.type.name}
                    className={`rounded-full px-2 py-0.5 text-xs font-medium text-white ${getTypeColor(type.type.name)}`}
                  >
                    {translateType(type.type.name)}
                  </span>
                ))}
              </div>
              <p className="mt-1 text-xs text-blue-500">#{stats.strongestPokemon.id.toString().padStart(3, "0")}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="border-blue-200 shadow-md">
        <CardHeader className="pb-2">
          <CardDescription>Maior Ataque</CardDescription>
          <CardTitle className="text-2xl capitalize">{stats.highestAttackPokemon.name}</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col items-center">
            <Flame className="mb-3 h-12 w-12 text-red-400" />
            <div className="text-center">
              <p className="text-sm text-blue-600">Pontos de Ataque:</p>
              <p className="text-xl font-bold text-blue-700">
                {stats.highestAttackPokemon.stats.find((s) => s.stat.name === "attack")?.base_stat || 0}
              </p>
              <div className="mt-1 flex justify-center gap-1">
                {stats.highestAttackPokemon.types.map((type) => (
                  <span
                    key={type.type.name}
                    className={`rounded-full px-2 py-0.5 text-xs font-medium text-white ${getTypeColor(type.type.name)}`}
                  >
              ...
              <p className="mt-1 text-xs text-blue-500">#{stats.highestAttackPokemon.id.toString().padStart(3, "0")}</p>
            </div>
          </div>
        </CardContent>
      </Card>
